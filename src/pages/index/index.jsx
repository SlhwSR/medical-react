...
<Header>
  <Menu theme={skin?"dark":"light"} mode="horizontal" defaultSelectedKeys={['index']} items={navheaer} className="ml-6" onClick={({item,key,keypath})=>switchtab(item,key,keypath)} />
...
<Layout>
  ...
  >
    ...
    >
     <span>点前定位：</span><Breadcrumb.Item>{bread}</Breadcrumb.Item>
    <div className="absolute right-10"><Switch className={skin?"h-8 w-10 bg-black":"h-8 w-10 bg-cyan-600"} onChange={()=>setskin(!skin)}>切换模式</Switch><span className="text-xl">换肤</span></div>
      </Breadcrumb>
    <Content
