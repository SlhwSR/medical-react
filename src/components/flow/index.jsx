...
const Flow = memo(() => {
  const [pagelist,setPagelist]=useState(["页面1"])
  ...
  return (
    ...
    <div className="w-60 h-screen inline-block bg-red-400">
      <div className="w-60 bg-slate-100" onClick={()=>showModal()}><FileAddFilled className="text-4xl cursor-pointer"></FileAddFilled></div>
       ...
       {(pagelist || []).map((item,index)=><List.Item className="text-center" style={{background:item===currentKey?"red":""}} onClick={()=>setCurrentKey(item)}  key={item}>{item}</List.Item>)}
       </List>
    </div>
    ...
    <Modal title="请输入页面名称" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <Input placeholder='输入页面名称' required={true} onChange={(val)=>setInput(val.target.value)} allowClear value={input}></Input>
    </Modal>
    </div>
  )
})
